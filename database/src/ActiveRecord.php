...
class ActiveRecord implements Iterator, ArrayAccess {
    ...
    public function __get($name) {
        $name = DB::symbolConvert($name);
        if (in_array($name, $this->columns)) {
            return $this->currentRecord[$name];
        }
        trigger_error(get_class($this->table) . " not exits property $name", E_USER_NOTICE);
    }
    ...
    protected function initKey() {
        if ($this->table->getKey()) {
            ...
            trigger_error(get_class($this->table) . " not exits key or unique", E_USER_NOTICE);
        }
    }
    ...
    public function offsetUnset($offset) {
        trigger_error('can not unset a property of table ' . get_class($this->table), E_USER_NOTICE);
    }
