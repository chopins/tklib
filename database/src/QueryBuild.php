...
use Toknot\Database\Column;
use Toknot\Database\Expression;
use Toknot\Database\LogicalExpression;
use Toknot\Database\FunctionExpression;
use Toknot\Database\QueryExpression;

class QueryBuild extends QueryExpression {
    ...
    /**
     *
     * @var \Toknot\Database\TableModel
     */
    ...
    public function bindColParameterValue(Column $col, $value) {
        $key = ':' . $col->getName() . count(self::$queryParameters);
     ...
     * @return $this
     */
    public function select($feild = '*', $table = null) {
    ...
    public function onAnd(...$args) {
        return $this->logicalExpression('AND', $args);
    }

    public function onOr(...$args) {
        return $this->logicalExpression('OR', $args);
    }

    public function onXor(...$args) {
        return $this->logicalExpression('XOR', $args);
    }
    ...
    public function findOne($id) {
        ...
        if (!$keyName) {
            ...
            if (!$unique) {
                throw new \PDOException('table ' . $this->tableModel->tableName() . ' not exits key', E_USER_WARNING);
            }
    ...
    protected function buildGroup() {
        ...
        return self::GROUP . self::LP . $this->buildFeild(array_keys($this->group)) . self::RP;
    }
    ...
    protected function setInsert($first, $data = []) {
        ...
        if (!is_numeric(current($keys))) {
            reset($keys);
            $this->leftFeildsList = self::LP . $this->buildFeild($keys) . self::RP;
        } else {
            $this->leftFeildsList = self::LP . $this->tableModel->getSelectFeild() . self::RP;
        }
        $len = count($first);
        $valueList = self::LP . join(',', array_fill(0, $len, '?')) . self::RP;
